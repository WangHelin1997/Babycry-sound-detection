...
class Evaluator(object):
    ...
    def evaluate(self, data_type, iteration, max_iteration=None, verbose=False):
        ...
        output = output_dict['output']  # (audios_num*28, in_domain_classes_num)
        target = output_dict['target']  # (audios_num*28, in_domain_classes_num)
...
#         print(np.sum(y_pred==y_true))
        ...
        rest = (len(y_true) - (l-1)*64*config.audio_num)//config.audio_num
        z_true = []
        z_pred = []
        ...
        logging.info('    Segment Average ccuracy: {:.3f}'.format(np.mean(segment_acc)))
        logging.info('    All Average ccuracy: {:.3f}'.format(np.mean(all_acc)))
